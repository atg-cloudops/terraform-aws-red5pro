...
module "red5pro" {
  ...
  path_to_red5pro_build        = "./red5pro-server-us-3fadfe20-d899-4a50-b5f3-be92e9245cbc.zip"      # Absolute path or relative path to Red5 Pro server ZIP file
  path_to_aws_cloud_controller = "./aws-cloud-controller-12.2.0.jar"          # Absolute path or relative path to AWS Cloud Controller JAR file


  # AWS authetification variables it use for Stream Manager autoscaling configuration
  aws_region     = "us-east-1"                                                # AWS region 
  aws_access_key = ""                                     # AWS IAM Access key (same as above)
  aws_secret_key = ""                                     # AWS IAM Secret key (same as above)

  # SSH key configuration
  ssh_key_create          = false                                             # true - create new SSH key, false - use existing SSH key
  ssh_key_name            = "red5key"                                         # Name for new SSH key or for existing SSH key
  ssh_private_key_path    = "/Users/kevin/Downloads/red5key.pem"              # Path to existing SSH private key
  
  # VPC configuration
  vpc_create              = false                                             # true - create new VPC, false - use existing VPC
  vpc_id_existing         = "vpc-0b3db6567d865161b"                           # VPC ID for existing VPC

  # MySQL DB configuration
  mysql_rds_instance_type = "db.t3.medium"                                    # Instance type for RDS instance
  mysql_user_name         = "red5pro"                                         # MySQL user name
  mysql_password          = "pro!5!redATG"                                    # MySQL password
  ...
  https_certificate_manager_certificate_name = "*.proxibid.com"               # Domain name for your SSL certificate

  # Stream Manager configuration 
  stream_manager_instance_type                = "t3.large"                    # Instance type for Stream Manager
  stream_manager_volume_size                  = 60                            # Volume size for Stream Manager
  stream_manager_api_key                      = "QR34dd2l01$fG6JC2i"          # API key for Stream Manager
  stream_manager_autoscaling_desired_capacity = 2                             # Desired capacity for Stream Manager autoscaling group
  stream_manager_autoscaling_minimum_capacity = 1                             # Minimum capacity for Stream Manager autoscaling group
  stream_manager_autoscaling_maximum_capacity = 12                            # Maximum capacity for Stream Manager autoscaling group
  ...
  red5pro_license_key           = "YU3M-CRQI-VPRE-Z3E6"                       # Red5 Pro license key (https://account.red5.net/login)
  red5pro_cluster_key           = "schooljungleparentthree"                   # Red5 Pro cluster key
  red5pro_api_enable            = true                                        # true - enable Red5 Pro server API, false - disable Red5 Pro server API (https://www.red5.net/docs/development/api/overview/)
  red5pro_api_key               = "rh3E4A$$hwd8hY8Taa"                        # Red5 Pro server API key (https://www.red5.net/docs/development/api/overview/)
  ...
  origin_image_instance_type                                = "t3.large"                          # Instance type for Origin node image
  ...
  node_group_origins_instance_type                = "t3.large"                # Instance type for Origins
  node_group_origins_capacity                     = 20                        # Connections capacity for Origins
  # Edge node configuration
  node_group_edges                                = 1                         # Number of Edges
  node_group_edges_instance_type                  = "t3.large"                # Instance type for Edges
  ...
  node_group_transcoders_instance_type            = "t3.large"                # Instance type for Transcoders
  ...
  tags = {
    ...
    Environment = "dev"
    Project     = "red5pro"
    deployed_by = "Kevin Mui"
    map-migrated = "d-server-03l5tp47a9jquk"
    deployment = "green"
  }
}
